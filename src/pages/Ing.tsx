...
function Ing({ navigation }: IngScreenProps) {
    ...
    useEffect(() => {
        Geolocation.getCurrentPosition(
            // Geolocation.watchPosition( // 계속 변하는 위치 감지
            info => {
                setMyPosition({
            ...
            console.error,
            {
                ...
                timeout: 20000,
                distanceFilter: 100, // 일정거리 이동했을떄 재호출
            },
        );
    }, []);
    ...
    if (!deliveries?.[0]) {
        return (
            <View style={{ alignItems: 'center', justifyContent: 'center', flex: 1 }}>
                <Text style={{ color: 'black' }}>주문을 먼저 받아주시지요</Text>
            </View>
        );
    }
    ...
    if (!myPosition || !myPosition.latitude) {
        return (
            <View style={{ alignItems: 'center', justifyContent: 'center', flex: 1 }}>
                <Text style={{ color: 'black' }}>위치 로딩 중입니다. 권한 허용하심? 체크점...</Text>
            </View>
        );
    }
            ...
            >
                ...
                >
                    ...
                    <Marker
                        ...
                        onClick={() => {
                            TMap.openNavi('출발지', end.longitude.toString(), end.latitude.toString(), 'MOTORCYCLE')
                            .then(data => {
                    ...
                    <Path
                        ...
                        color="orange"
                        onClick={() => {
                            TMap.openNavi('도착지', end.longitude.toString(), end.latitude.toString(), 'MOTORCYCLE')
                            .then(data => {
                                console.log('TMap callback', data);
                                if (!data) {
                                    Alert.alert('알림', '티맵을 설치하세요.');
